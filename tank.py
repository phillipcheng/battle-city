...
class Tank():
    ...
    def __init__(self, level, side, position=None, direction=None, filename=None, globals=None, speed_in=None,
                 super_power=None):
        ...
        self.max_active_bullets = 1 #all directions
        ...
        # 3 - can destroy steel
        if super_power is not None:
            self.superpowers = super_power
        else:
            self.superpowers = 0
        ...
        self.controls = [pygame.K_SPACE, pygame.K_UP, pygame.K_RIGHT, pygame.K_DOWN, pygame.K_LEFT,
                         pygame.K_o, pygame.K_p, pygame.K_l, pygame.K_COMMA]
    ...
    def explode(self):
        ...
        if self.state != self.STATE_DEAD:
            ...
            if self.bonus:
                self.spawnBonus()
    def gen_bullet(self, direction):

        bullet = Bullet(self.level, self.rect.topleft, direction, globals=self.globals)
    ...
    def fire(self, forced=False, allow_full_fire=False, all_direction=False):
        """ Shoot a bullet
        ...
        if not forced:
            ...
            for bullet in self.globals.bullets:
                if bullet.owner_class == self and bullet.state == bullet.STATE_ACTIVE:
                    active_bullets += 1
            if allow_full_fire is False:
                if active_bullets >= self.max_active_bullets:
                    return False

        if all_direction is False:
            self.gen_bullet(self.direction)
        else:
            self.gen_bullet(self.DIR_UP)
            self.gen_bullet(self.DIR_DOWN)
            self.gen_bullet(self.DIR_LEFT)
            self.gen_bullet(self.DIR_RIGHT)
    ...
    def bulletImpact(self, friendly_fire=False, damage=100, tank=None):
        ...
        if self.side == self.SIDE_ENEMY:
            ...
            # 	self.timer_uuid_paralise = self.timer.add(1000, lambda :self.setParalised(False), 1)
            return True
